...
export class MarginAccount {
  ...
  async loadOpenOrders(connection: Connection, dexProgramId: PublicKey): Promise<(OpenOrders | undefined)[]> {
  ...
  toPrettyString(mangoGroup: MangoGroup): string {
    const lines = [`MarginAccount: ${this.publicKey.toBase58()}`, `Asset Deposits Borrows`];

    const tokenNames = ['BTC', 'ETH', 'USDC']; // TODO pull this from somewhere
    for (let i = 0; i < mangoGroup.tokens.length; i++) {
      lines.push(`${tokenNames[i]} ${this.getUiDeposit(mangoGroup, i)} ${this.getUiBorrow(mangoGroup, i)}`);
    }

    return lines.join('\n');
  }

  async getValue(connection: Connection, mangoGroup: MangoGroup): Promise<number> {
  ...
  async cancelAllOrdersByMarket(
    ...
    return await Promise.all(
      orders.map((order) => client.cancelOrder(connection, programId, mangoGroup, this, owner, market, order)),
    );
  }
}
...
export class MangoClient {
  ...
  async initMarginAccount(
    ...
    const accInstr = await createAccountInstruction(connection, owner.publicKey, MarginAccountLayout.span, programId);
  ...
  async settleAll(
    ...
    for (let i = 0; i < NUM_TOKENS; i++) {
      ...
      const data = encodeMangoInstruction({
        SettleBorrow: { tokenIndex: new BN(i), quantity: uiToNative(liabs[i] * 2, mangoGroup.mintDecimals[i]) },
      });
  ...
  async getMangoGroup(connection: Connection, mangoGroupPk: PublicKey): Promise<MangoGroup> {
